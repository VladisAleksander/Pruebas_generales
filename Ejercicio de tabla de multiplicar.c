...
int main() {
    ...
    for( i = 0; i < fila; i++){ // Ajustamos el número de filas
        for( j = 0; j < col; j++){ // Ajustamos el número de columnas
            matriz[fila][col] = (i+1) * (j+1); // Multiplicamos los números de las filas por las columnas para ingresarlos a la matriz
            printf("| %3d |", matriz[fila][col]); // Imprimir la matriz con el resultado final
        }
        printf("\n");
    }
